<?php
class knl_extensions_dnfsaida_criteria extends knl_lib_daoext_Convert {
	...
	public function montaSql($arrayFiltro){
		...
		$this->sql = "";
	  	$this->innerJoin .= " LEFT JOIN d_nf_saida ON (tb.id = d_nf_saida.id_doc) ";
	    $this->innerJoin .= " LEFT JOIN d_cad_nf d_nf_fornecedor ON (d_nf_fornecedor.id = d_nf_saida.id_fornecedor) ";
	    $this->orderBy = array("d_nf_saida.datasai","d_nf_fornecedor.razao","tb.numero");
	    if (!empty($arrayFiltro['data2'])){
	  	   $this->sql .= " AND d_nf_saida.datasai <= ? ";
