<?php
class knl_extensions_dpedcompra_dao extends knl_extensions_cadastronf_CadParent {
    private $conn;
    ...
    public function selectById($id) {
       ...
       if($l = $stmt->FetchRow()) {
          $objmodel = new knl_extensions_dpedcompra_model($l['id'],$l['id_doc'],$l['id_fornecedor']);
       } else {
            throw new Exception("Nenhum d_ped_compra foi encontrado!");
         }
       return $objmodel;
    }
    ...
    public function getFullData($mDocH){
    	...
    	$PedEntrega = knl_extensions_dpedcompra_daoPedCompraEntrega::getInstance();
    ...
    public function upsert(knl_extensions_dpedcompra_model $objmodel){
       if ($objmodel->get_id() == 0){
          $query = "INSERT INTO d_ped_compra (id_doc,id_fornecedor)
                    VALUES ('".$objmodel->get_id_doc()."','".$objmodel->get_id_fornecedor()."')";
          $stmt = $this->conn->prepare($query);
          $stmt = $this->conn->execute($stmt);
          $objmodel->set_id($this->conn->Insert_ID());
       } else {
            $query = "UPDATE d_ped_compra SET 
