<?php
class knl_extensions_dcotvenda_dao {
    private $conn;
    ...
    public function selectById($id) {
       ...
       if($l = $stmt->FetchRow()) {
          $objmodel = new knl_extensions_dcotvenda_model($l['id'],$l['id_doc'],$l['id_fornecedor']);
       } else {
            throw new Exception("Nenhum d_cot_venda foi encontrado!");
         }
       return $objmodel;
    }
    ...
    public function upsert(knl_extensions_dcotvenda_model $objmodel){
       if ($objmodel->get_id() == 0){
          $query = "INSERT INTO d_cot_venda (id_doc,id_fornecedor)
                    VALUES ('".$objmodel->get_id_doc()."','".$objmodel->get_id_fornecedor()."')";
          $stmt = $this->conn->prepare($query);
          $stmt = $this->conn->execute($stmt);
          $objmodel->set_id($this->conn->Insert_ID());
       } else {
            $query = "UPDATE d_cot_venda SET 
	...
	public function getFullData($param){
        $arrModel = array();
        $arrModel["d_cotacao_cli"]= knl_extensions_cadastronf_cotDao::getInstance()->selectById($param->get_id_fornecedor());
        return $arrModel;
    }
}
?>
